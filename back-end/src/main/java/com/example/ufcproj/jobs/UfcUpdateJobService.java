...
import com.example.ufcproj.service.S3ImageService;
...
public class UfcUpdateJobService {
    ...
    @Transactional
    public void convertToS3Url() throws IOException {
        List<Fighter> fighters = fighterRepository.findAll();
        for(int i = 0; i < fighters.size(); i++){
            s3ImageService.convertFighterImageToS3(fighters.get(i));
            System.out.println("Fighter " + (i + 1) + " of " + fighters.size() + " added");
        }
        System.out.println("done");
    }
