...
public class UfcUpdateJobService {
    ...
    public void runPollToday() throws InterruptedException, IOException {
        ...
        if(fights.isEmpty()){
            ...
            runEventPollRefresh(upcomingEvent, today);
                ...
                while(!fightConnected){
                    try{
                        ...
                        if(!fightPage.select("i.b-fight-details__person-status_style_gray").isEmpty()){
                            ...
                            if(fightWinner != null && fightWinner.contains(fight.getRedFighterId().getFirstName()) && fightWinner.contains(fight.getRedFighterId().getLastName())){
                                fight.setWinnerCorner(Fight.WinnerCorner.RED);
                            } else if(fightWinner != null && fightWinner.contains(fight.getBlueFighterId().getFirstName()) && fightWinner.contains(fight.getBlueFighterId().getLastName())){
            ...
            while(!eventConnected){
                try{
                    ...
                    for(int x = 0; x < eventFights.size(); x++){
                        ...
                        String redStat = eventFights.get(x).select("td:nth-child(2) p:nth-child(1) a").attr("abs:href").split("/")[4];
                        String blueStat = eventFights.get(x).select("td:nth-child(2) p:nth-child(2) a").attr("abs:href").split("/")[4];
    ...
    @Transactional
    public void runEventPollRefresh(Event upcomingEvent, LocalDate today) throws IOException, InterruptedException {
        boolean eventConnected = false;
        while(!eventConnected){
            try{
    ...
    @Transactional
    public static void getUpcomingEvents(EventRepository eventRepository) throws IOException {
    ...
    public void handleFightChange(Fight fight){
        List<Pick> fightPicks = picksRepository.findByFightId(fight.getFightId());
